...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: true, maintenance: rows.map(serializeMaintenance) });
  } catch (error) {
    console.error("GET /api/operations/maintenance failed:", error);
    return NextResponse.json({ success: false, message: "Failed to load maintenance records." }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const maintenanceType = String(body.maintenance_type ?? "SCHEDULED").toUpperCase();
    ...
    if (!vehicleId || !serviceDate || !nextServiceDate || !Number.isFinite(cost)) {
      return NextResponse.json({ success: false, message: "Vehicle, dates, and cost are required." }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, maintenance: serializeMaintenance(record) }, { status: 201 });
  } catch (error) {
